...
public class ReferenceUpdater implements IReferenceUpdater {
	...
	protected boolean containsReferenceText(Delta delta, IUpdatableReference ref) {
		String text = ref.getReferenceRegion().getText();
		QualifiedName exp = converter.toQualifiedName(text);
	...
	protected boolean needsUpdating(IReferenceUpdaterContext context, IUpdatableReference ref) {
		...
		if (delta != null && !containsReferenceText(delta, ref)) {
			return true;
		}
